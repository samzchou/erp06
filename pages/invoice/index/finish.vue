    ...
    <section class="list-container">
        ...
        <div class="table-content">
            <el-table v-loading="listLoading" ref="detailStore" :data="gridList" border fit highlight-current-row stripe size="mini"
                max-height="500" style="width:100%" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="50" align="center" />
                <el-table-column type="expand">
                    <template slot-scope="scope">
                        <el-row :gutter="20" v-for="(item,idx) in scope.row.result" :key="idx">
                            <el-col :span="4">
                                <span style="width:30px">{{idx+1}}、</span>项目号：{{item.projectNo}}</el-col>
                ...
                <el-table-column prop="allPrice" label="开票金额" width="120">
                    <template slot-scope="scope">
                        <span>{{scope.row.allPrice | currency("",4)}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="payPrice" label="总金额(含税)" width="120">
                    <template slot-scope="scope">
            ...
            <div class="page-container">
                <div>
                    ...
                    <el-button size="mini" type="danger" @click="exportXML" :disabled="!selectRows.length || exportLoading" :icon="exportLoading?'el-icon-loading':'el-icon-download'">导出XML
                    </el-button>
                </div>
        ...
        <el-dialog class="invoice-dialog" title="开票详情" append-to-body :close-on-click-modal="false" :visible.sync="openDialogVisible"
            width="75%">
            <div>
                <el-table :data="currOrders" border fit highlight-current-row stripe size="mini" max-height="450" style="width:100%">
    ...
    methods: {
            ...
            this.getOrders(ids).then(res => {
                ...
                res.forEach(order => {
                    let obj = {
                        ...
                        "Lslbz": "",
                        "Yhzcsm": "",
                        "Dj": order.price, // 需要做含税金额？
                        "Sl": order.count,
                        "Je": order.price * order.count, // 需要做含税金额？
        ...
        submitExport() {
            ...
            import('@/components/Export2Excel').then(excel => {
                ...
                const filterVal = ['createDate', 'createByUser', 'invoiceNumber', 'slv', 'slvMoney', 'allPrice', 'payPrice'];
        ...
        async getList(match = {}) {
            ...
            this.gridList = result.list.map(item => {
                ...
                item.slvMoney = item.allPrice * this.dsCrm.slv / 100; // 税额
                item.payPrice = item.allPrice + item.slvMoney; // 订单合计总金额（含税）
                return item;
            });
