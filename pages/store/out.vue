<template>
	<section>
		...
		<div class="grid-container">
			...
			<div>
				<el-table v-loading="listLoading" ref="detailStore" :data="gridList" border fit highlight-current-row stripe size="mini" max-height="500" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="库存量计算" width="100">
						<template slot-scope="scope">
        ...
        <el-dialog title="修改交货日期" append-to-body :visible.sync="editVisible" width="300px" @close="editRow=null">
            <div v-if="editRow">
                <el-date-picker size="mini" value-format="timestamp" v-model="editRow.deliveryDate" type="date" :clearable="false" placeholder="选择日期" style="width:150px;"/>
	...
	methods: {
        calcStore(arr){
            ...
            for(let i=0; i<arr.length; i++){
                if(arr[i]['count'] > arr[i]['storeCount']){
                    flag = true;
                    break;
                }
            }
		...
		async getList(match = {}) {
            ...
            //console.log('this.storeArr', storeArr);
