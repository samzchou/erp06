<template>
    <section class="order-main">
        ...
        <el-dialog title="采购订单仓库入库清单" append-to-body :close-on-click-modal="false" :visible.sync="openOrderVisible" width="85%">
            ...
            <div>
                <el-table ref="exportTable" :data="crmOrderList" border fit highlight-current-row stripe size="mini" max-height="350" v-loading="searchLoading"
                    ...
                    <el-table-column prop="metaprice" label="采购单价" width="100">
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-input-number v-if="!scope.row.isAdded" size="mini" controls-position="right" :min="0" :step="0.1" @change="changeMetaPrice(scope.row)"
                                    v-model="scope.row.metaprice" style="width:80" />
                    ...
                    <el-table-column prop="incount" label="实际入库数量" width="100">
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-input-number v-if="!scope.row.isAdded" size="mini" controls-position="right" :min="0" @change="changeInCount(scope.row)"
                                    v-model="scope.row.incount" style="width:80" />
