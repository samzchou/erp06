<template>
    <section class="order-main">
        ...
        <div class="grid-container">
            <div class="search-content">
                <el-form :inline="true" :model="searchForm" ref="searchForm" size="mini" @keyup.native.enter="submitSearch">
                    ...
                    <el-form-item label="制单日期：" prop="createDate">
                        <el-date-picker v-model="searchForm.createDate" value-format="timestamp" type="daterange" range-separator="至" start-placeholder="开始日期"
                            end-placeholder="结束日期" clearable editable unlink-panels style="width:250px" />
                    </el-form-item>
                    <el-form-item label="交货日期：" prop="finishedDate">
                        <el-date-picker v-model="searchForm.finishedDate" value-format="timestamp" type="daterange" range-separator="至" start-placeholder="开始日期"
                            end-placeholder="结束日期" clearable editable unlink-panels style="width:250px" />
                    </el-form-item>
                    <el-form-item>
            ...
            <el-table class="table-container" ref="listTable" v-loading="listLoading" :data="gridList" border fit highlight-current-row
                stripe size="mini" max-height="500">
...
>>>>>>> 96229637a28dbc57d9f0dff1bdf2ca241d6b8a51
        ...
        <el-dialog title="供应商采购清单" append-to-body :visible.sync="openOrderVisible" width="85%">
                    ...
                    <el-table-column prop="metaprice" label="采购单价" width="100">
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-input-number size="mini" v-if="!scope.row.isAdded" controls-position="right" :min="0" :step="0.1" v-model="scope.row.metaprice"
                                    @change="setRowData(scope.row)" style="width:80" />
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <span v-if="!scope.row.isAdded">
                                    <el-input-number size="mini" controls-position="right" :min="scope.row.count" v-model="scope.row.incount" @change="setRowData(scope.row)"
                                        style="width:80px" />
                                </span>
            ...
            <div class="btns" v-if="crmOrderList.length">
                <div>
                    <el-pagination size="mini" @size-change="handleSizeOrder" @current-change="handleCurrentOrder" :current-page.sync="queryIn.page"
                        :page-sizes="[3,20, 50, 100, 200,500]" :page-size="queryIn.pagesize" layout="total,sizes,prev,pager,next"
                        :total="queryInTotal" />
                </div>
                <div>
                    <span>采购制单交货日期：</span>
                    <el-date-picker size="small" v-model="currItem.finishedDate" value-format="timestamp" :clearable="false" type="date" placeholder="选择日期"
...
export default {
    ...
    methods: {
		...
		async handleDeleteBySerial(row) {
			...
			this.$confirm('将同时删除所有' + result.total + '条订单，并影响到实际出库！确定删除？', '提示：请谨慎操作此删除', {
		...
		*/
        handleDeleteOrder(row) {
            ...
            debugger
            this.$confirm('确定移除该条订单，同时将删除' + row.orderIds.length + '条原订单，并影响到实际出库！确定删除？', '从供应商订单汇总中移除订单', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                ...
                this.$axios.$post('mock/db', { data: condition }).then(result => {
                    ...
                    this.removeSourceOrder(row.orderIds);
                    this.submitSearch(true);
                });
            }).catch(() => { });
        },
        removeSourceOrder(ids) {
            let condition = {
                type: 'removePatch',
