<template>
    <section class="order-main">
        ...
        <div class="grid-container" v-if="!isEdit">
            ...
            <el-table class="table-container" v-loading="listLoading" :data="gridList" border fit highlight-current-row stripe size="mini"
                max-height="500">
                ...
                <el-table-column type="expand">
                    <template slot-scope="props">
                        <el-row :gutter="20" v-for="(item,idx) in props.row.result.filter(o=>o.typeId==1)" :key="item.id">
                            <el-col :span="3" :class="{'warning':item.isCanceled}">
                            ...
                            <el-col :span="5">
                                <el-button type="text" @click="showDetail(item)">供应商：{{item.crmName}}</el-button>
                            </el-col>
                        </el-row>
                    </template>
                </el-table-column>
                <el-table-column :label="needSource?'蒂森订单号':'珏合订单号'" width="120">
                    <template slot-scope="scope">
...
>>>>>>> 96229637a28dbc57d9f0dff1bdf2ca241d6b8a51
                ...
                <el-table-column label="操作" fixed="right" align="center" :width="needSource?'100':'120'">
                    <template slot-scope="scope">
                        ...
                        <!-- <el-button size="mini" type="text" icon="el-icon-document" @click="setOrder(scope.row)">制单</el-button> -->
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="'供应商采购清单--'+(needSource?'蒂森采购清单':'珏合采购清单')" append-to-body :close-on-click-modal="false" :visible.sync="openDialogVisible"
            width="90%">
            ...
            <div class="compare" id="printTable" ref="printTable">
                ...
                <el-table ref="exportTable" :data="crmOrderList" border fit highlight-current-row stripe size="mini" max-height="350" v-loading="searchLoading"
                    @selection-change="handleSelectionOrders" style="width:100%">
...
>>>>>>> 96229637a28dbc57d9f0dff1bdf2ca241d6b8a51
                    ...
                    <!-- <el-table-column prop="rcount" label="已制单未出库" width="100" /> -->
                    <el-table-column prop="cancelTotal" label="已取消" width="70">
                    ...
                    <el-table-column prop="releaseCount" label="实际采购量" width="100">
                        <template slot-scope="scope">
                            <div v-if="scope.$index<crmOrderList.length-1">
                                <el-input-number size="mini" controls-position="right" :min="0" v-model="scope.row.releaseCount" @change="checkReleaseCount(scope.row,crmOrderList)"
                                    style="width:80px" />
                            </div>
...
>>>>>>> 96229637a28dbc57d9f0dff1bdf2ca241d6b8a51
        ...
        dialogSearch() {
            ...
            for (let k in this.dialogForm) {
                if (this.dialogForm[k] != "" && this.dialogForm[k]) {
                    ...
                    } else if (_.isArray(this.dialogForm[k]) && (k === "orderDate" || k === "deliveryDate")) {
                        params[k] = {
        ...
        async listOrderByCrm(match = {}) {
            ...
            match = _.merge({ flowStateId: 1 }, bySerial, match); //{ $lt: 9 }
            let params = {
                type: "aggregate",
                collectionName: "order",
                data: match,
            ...
            console.log('listOrderByCrm', data);
        ...
        mergeOrder(lists, sdata) {
            ...
            lists.forEach(item => {
                ...
                if (!!~dataIndex) {
                    ...
                    //let otherItems = this.getOtherItem(item);
                    listData.push(item);
                }
            });
        ...
        //count: { $first: "$count" }, 
        async getOtherItem(item) {
            let match = { "materialNo": item.materialNo, "flowStateId": { $gt: 2, $lt: 4 } };
            let params = {
                type: "aggregate", //groupList aggregate getColumns
                collectionName: "order",
                data: match,
                aggregate: [
                    { $match: match },
