<template>
	<section class="order-main">
                ...
                <el-table-column type="expand">
                    <template slot-scope="props">
                        <el-row :gutter="20" v-for="(item,idx) in props.row.result.filter(o=>o.typeId==2)" :key="item.id">
                            <el-col :span="2" :class="{'warning':item.isCanceled}"><span style="width:30px">{{idx+1}}.</span>状态：{{item.isCanceled?'已取消':'正常'}}</el-col>
                            ...
                            <el-col :span="4"><el-button type="text" @click="showDetail(item)">客户：{{item.crmName}}</el-button></el-col>
                        </el-row>
                    </template>
                </el-table-column>
		...
		<!--制定全部订单-->
		<el-dialog :title="needSource?'蒂森生产订单':'珏合生产订单'" append-to-body :close-on-click-modal="false" :visible.sync="openDialogVisible" width="90%">
		...
		dialogSearch() {
			...
			if (this.dialogForm.orderDate) {
				params.orderDate = {
					...
					$lte: this.dialogForm.orderDate[1] + 24*3600*1000
                };
            }
            if (this.dialogForm.deliveryDate) {
                params.deliveryDate = {
                    $gte: this.dialogForm.deliveryDate[0],
					$lte: this.dialogForm.deliveryDate[1] + 24*3600*1000
                };
			}
